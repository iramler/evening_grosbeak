...
df.block.1 <- colldata %>%
  filter(motusTagID == 24350, 
         recvDeployName %in% "West Port Bruce") %>% 
...
df.block.2 <- colldata %>%
  filter(motusTagID == 24363, 
         recvDeployName %in% "Koffler") %>% 
  select(motusTagID, runID) %>%
  distinct() %>%
  collect()

########### unlikely detections in July and June
df.block.3 <- colldata %>%
  filter(motusTagID == 24366) %>%
  select(motusTagID, runID) %>%
  distinct() %>%
  collect()
